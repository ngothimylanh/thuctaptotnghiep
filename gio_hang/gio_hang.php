...
?>
		...
		<?php
            ...
            $sql_update = "UPDATE san_pham SET soluong = soluong - $sp WHERE masp = $masp";
            $exec_update = mysqli_query($connect, $sql_update);
            if (!$exec_update) {
                echo "Lỗi cập nhật dữ liệu: " . mysqli_error($connect);
            }
        }
...
?>
	<tr>
		...
		<td width="15%">
		<input type="number" min="1" class="form-control" style="width: 70%;" max="<?php echo $row['soluong'];?>" name="sl_sp[<?php echo $row['masp']; ?>]" value="<?php echo $_SESSION['giohang'][$row['masp']]; ?>">
		</td>
